...
//   // Add state variables for role and email
...
import { FaCheck } from "react-icons/fa";
import { FormControl } from "react-bootstrap";
import { useSelector } from "react-redux"; // For accessing currentUser
...
//   const { uid } = useParams();
  ...
  // Get currentUser from Redux store (logged-in user)
  const { currentUser } = useSelector((state: any) => state.accountReducer);
  const isAdmin = currentUser?.role === "ADMIN"; // Check if the logged-in user is an admin
...
>
  <IoCloseSharp className="fs-1" />
      ...
      {/* Delete and Cancel buttons only visible for Admin */}
      {isAdmin && (
        <div className="mt-3 pt-3">
          <button
            onClick={() => deleteUser(uid)}
